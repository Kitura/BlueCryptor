...
Due to an issue with API differences between the `Foundation` implementation on **macOS** versus the implementation on **Linux**, the following API usuage is not available on **Linux**.  It remains available on **macOS**.  Once the issue is resolved, this API usage will be available on *both* platforms. This API is an extension to `Data` that allows generation of a `digest` from a previously populated `Data` instance.
...
let theData: Data = shaShortBlock.data(using:NSUTF8StringEncoding)!
...
```
These tests pass on **macOS** but will fail to compile on **Linux** as this extension is not implemented currently for **Linux**.
